...
--		ShowMissingBlessing = true,
		...
		HideInCombat = true,
		usepallypower = false,
		dumbassignment = false,
		noppifpaladinmissing = false,
		...
		misdirectionraid = true,
		misdirectionparty = true,
		deathwarn = true,
		tankdeathself = true,
		tankdeathsound = true,
		tankdeathrw = false,
			...
			if type(BF[buffcheck].chat) == "string" then
				if # report[BF[buffcheck].list] > 0 then
					if BF[buffcheck].timer then
						...
						local tbl = RaidBuffStatus:SafeConcatTable(timerlist)
						RaidBuffStatus:Say("<" .. BF[buffcheck].chat .. ">: " .. tbl)
						--RaidBuffStatus:Say("<" .. BF[buffcheck].chat .. ">: " .. table.concat(timerlist, ", "))
					else
						local tbl = RaidBuffStatus:SafeConcatTable(report[BF[buffcheck].list])
						RaidBuffStatus:Say(prefix .. "<" .. BF[buffcheck].chat .. ">: " .. tbl)
...
local NAMES_ANNOUNCED_LIMIT = 5
function RaidBuffStatus:SafeConcatTable(tbl)
	if #tbl > NAMES_ANNOUNCED_LIMIT then
		local tblstr = ""..tbl[1]
		local index = 1
		while (index < NAMES_ANNOUNCED_LIMIT)
		do
			index = index + 1
			tblstr = tblstr..", "..tbl[index]
		end
		return tblstr.."... ("..(#tbl)..")"
	else
...
function RaidBuffStatus:WhisperBuff(buff, report, raid, prefix)
	DEFAULT_CHAT_FRAME:AddMessage("Function disabled.")
	--[[
...
function RaidBuffStatus:MisdirectionEventLog(srcname, spellname, dstname)
	...
	if RaidBuffStatus.db.profile.misdirectionraid then
		SendChatMessage(L["%s cast %s on %s"]:format(srcname, spellname, dstname), "raid")
	end
	if RaidBuffStatus.db.profile.misdirectionparty and (not raid.israid) then
		SendChatMessage(L["%s cast %s on %s"]:format(srcname, spellname, dstname), "party")
	end
end
...
local function IsBuffCasterPresentRecorded(buffcasters, buffId, unitList)
	for _, unit in pairs(unitList) do
		local buff = unit.hasbuff[BS[buffId]]
		if buff and buff.caster and not buffcasters[buff.caster] then
			return buff.caster
		end
	end
	return nil
...
function RaidBuffStatus:GetBuffCasters(buffsToCheck, unitList)
	local buffcasters = {}
	for _, buffId in ipairs(buffsToCheck) do
		local buffcaster = IsBuffCasterPresentRecorded(buffcasters, buffId, unitList)
		if buffcaster ~= nil then
			table.insert(buffcasters, buffcaster)
			break
		end
	end
	return buffcasters
...
function RaidBuffStatus:UpdateNoBuffList(unitName, unitBuffList, buffsToCheck, nobufflist)
	local hasAnyBuffs = false
	for _, buffId in ipairs(buffsToCheck) do
		if unitBuffList[BS[buffId]] then
			hasAnyBuffs = true
			break;
		end
	end
	if not hasAnyBuffs then
		table.insert(nobufflist, unitName)
	end
end
